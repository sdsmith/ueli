...
const contextBridgeImplementation: ContextBridge = {
    ...
    autostartSettingsChanged: (autostartIsEnabled) => ipcRenderer.send("autostartSettingsChanged", { autostartIsEnabled }),
    ...
    getExtensionAssetFilePath: (extensionId, key) => ipcRenderer.sendSync("getExtensionAssetFilePath", { extensionId, key }),
    getExtensionSettingDefaultValue: (extensionId, settingKey) => ipcRenderer.sendSync("getExtensionSettingDefaultValue", { extensionId, settingKey }),
    getFavorites: () => ipcRenderer.sendSync("getFavorites"),
    getLogs: () => ipcRenderer.sendSync("getLogs"),
    getInstantSearchResultItems: (searchTerm: string) => ipcRenderer.sendSync("getInstantSearchResultItems", { searchTerm }),
    getOperatingSystem: () => ipcRenderer.sendSync("getOperatingSystem"),
    getSearchResultItems: () => ipcRenderer.sendSync("getSearchResultItems"),
    getSettingsFile: () => ipcRenderer.sendSync("getSettingsFile"),
    getSettingValue: (key, defaultValue, isSensitive) => ipcRenderer.sendSync("getSettingValue", { key, defaultValue, isSensitive }),
    ...
    removeExcludedSearchResultItem: (itemId: string) => ipcRenderer.invoke("removeExcludedSearchResultItem", { itemId }),
    ...
    updateSettingValue: (key, value, isSensitive) => ipcRenderer.invoke("updateSettingValue", { key, value, isSensitive }),
};
